...
| 63 | current slice | Camera HLS stats application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy stats 欄位 | RED failed with missing `schema_version`; camera tests `113 passed`; full suite `546 passed` |
...
| Camera path | camera list/register/unregister/clear-cache/config-list/HLS start/info/stats application response envelope，保留 legacy camera list body、stats、config success/list、HLS payload 與 stream info 欄位 | `b174ee2`, `1531478`, `b42d26a`, `7660fb8`, `9ef6f75`, `77665f5`, `ede433d`, current slice |
