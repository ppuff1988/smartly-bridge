...
| 87 | `current slice` | WebRTC offer signaling-failure application response 補上 API vNext `schema_version`、`data.status`、`warnings`、`errors[]` envelope 欄位，同時保留 legacy `error` / `message` / `session_id` 欄位與 500 status | RED failed with missing `schema_version`; WebRTC tests `51 passed`; full suite `558 passed` |
...
- Targeted WebRTC offer signaling-failure vNext envelope test: `1 passed`
