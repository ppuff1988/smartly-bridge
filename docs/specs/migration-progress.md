...
| 90 | `current slice` | History batch application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy `history` / `count` / `truncated` / `denied_entities` / metadata 欄位 | RED failed with missing `schema_version`; targeted test `1 passed`; history tests `44 passed`; full suite `558 passed` |
...
| History path | history invalid-time-range、time-range-too-large、single-query 與 batch application response envelope，保留 legacy `error` / `max_days` / history payload 欄位 | `4979988`, `be5a1e5`, `296da10`, `current slice` |
...
- Targeted history batch vNext envelope test: `1 passed`
