...
| 81 | `current slice` | WebRTC token application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy token / endpoint / ICE 欄位 | RED failed with missing `schema_version`; WebRTC tests `51 passed`; full suite `558 passed` |
...
| WebRTC path | WebRTC token response envelope 與 ICE session-not-found/entity-mismatch、hangup session-not-found/entity-mismatch application response envelope，保留 legacy token / endpoint / ICE 與 `error` 欄位 | `ff78eb5`, `8d56e3e`, `93a2ee5`, `fc083e3`, `current slice` |
