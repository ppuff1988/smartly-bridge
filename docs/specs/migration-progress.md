...
| 83 | `current slice` | WebRTC hangup closed application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy `status` 欄位 | RED failed with missing `schema_version`; WebRTC tests `51 passed`; full suite `558 passed` |
...
| WebRTC path | WebRTC token response envelope、ICE accepted success envelope、hangup closed success envelope 與 ICE session-not-found/entity-mismatch、hangup session-not-found/entity-mismatch application response envelope，保留 legacy token / endpoint / ICE、`status` / `candidates` 與 `error` 欄位 | `ff78eb5`, `8d56e3e`, `93a2ee5`, `fc083e3`, `4e27a90`, `97a13dd`, `current slice` |
...
- Targeted WebRTC hangup closed vNext envelope test: `1 passed`
