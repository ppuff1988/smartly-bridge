...
| 78 | `current slice` | Legacy control service-not-allowed application response 補上 API vNext `schema_version`、`data.status`、`warnings`、`errors[]` envelope 欄位，同時保留 legacy `error` 欄位與 403 status | RED failed with missing `schema_version`; affected tests `111 passed`; full suite `557 passed` |
...
| Command path | 新增 canonical `SmartlyCommand` dispatcher、target resolver、expected state、standard error shape，並為 command success/error 與 legacy control entity/service deny 補上 API vNext envelope/error fields | `564c8c4`, `2dd37ac`, `edb4a68`, `df54f35`, `a073269`, `a094b98`, `eaad20a`, `current slice` |
...
- Targeted legacy control service-not-allowed vNext error envelope test: `1 passed`
- Affected control application/HTTP tests: `111 passed`
