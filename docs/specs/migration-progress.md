...
| 94 | `current slice` | Camera snapshot 304 與 MJPEG stream response 固定為非 JSON envelope 例外，分別輸出 `X-Smartly-Response-Mode: empty` / `stream` 並保留空 body / streaming headers | RED failed with missing response-mode headers; targeted tests `3 passed`; camera tests `122 passed`; full suite `558 passed` |
...
- Targeted camera non-JSON response-mode tests: `3 passed`
