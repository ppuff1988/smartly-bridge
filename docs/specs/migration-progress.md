...
| 75 | `current slice` | WebRTC hangup session-not-found application response 補上 API vNext `schema_version`、`data.status`、`warnings`、`errors[]` envelope 欄位，同時保留 legacy `error` 欄位與 404 status | RED failed with missing `schema_version`; WebRTC tests `50 passed`; full suite `555 passed` |
...
| WebRTC path | WebRTC ICE session-not-found/entity-mismatch 與 hangup session-not-found application response envelope，保留 legacy `error` 欄位 | `ff78eb5`, `8d56e3e`, `current slice` |
...
- Targeted WebRTC hangup session-not-found vNext error envelope test: `1 passed`
- Affected WebRTC application/view tests: `50 passed`
