...
| 95 | `current slice` | Sync states logical-device read path 在 API vNext `data` 內同步輸出 `read_path` / `devices` / `device_count`，讓 vNext client 不需讀 top-level legacy 欄位即可切換 read path | RED failed with missing `data.read_path`; targeted tests `2 passed`; affected sync/hexagonal tests `102 passed`; full suite `558 passed` |
...
| Sync aliases, warnings, and read path | structure/states response envelope、logical devices migration aliases、normalization warnings，並支援 `use_logical_devices` read-path flag；logical-device read path 已同步到 top-level 與 API vNext `data` payload | `e47050c`, `040f769`, `4527bd5`, `14f5de7`, `aad30d2`, `current slice` |
...
- Targeted sync logical read-path vNext data tests: `2 passed`
- Affected sync/hexagonal tests: `102 passed`
