...
| 73 | `current slice` | WebRTC ICE session-not-found application response 補上 API vNext `schema_version`、`data.status`、`warnings`、`errors[]` envelope 欄位，同時保留 legacy `error` 欄位與 404 status | RED failed with missing `schema_version`; WebRTC tests `48 passed`; full suite `553 passed` |
...
| WebRTC path | WebRTC ICE session-not-found application response envelope，保留 legacy `error` 欄位 | `current slice` |
...
- Targeted WebRTC ICE session-not-found vNext error envelope test: `1 passed`
- Affected WebRTC application/view tests: `48 passed`
