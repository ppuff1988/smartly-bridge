...
- Device event dispatch failure envelope RED: targeted test failed with legacy-only `{"error": "device_event_failed", "message": "RuntimeError: event bus unavailable"}`
- Targeted device event dispatch failure envelope test: `1 passed`
- Device event tests: `28 passed`
- Affected automation/event/http tests: `101 passed`
- Full suite: `604 passed` on Python 3.14.6 / `mcr.microsoft.com/devcontainers/python:3.14-bookworm`
