...
| 61 | current slice | Camera HLS start application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy HLS payload 欄位 | RED failed with missing `schema_version`; camera tests `111 passed`; full suite `544 passed` |
...
| Camera path | camera list/register/unregister/clear-cache/config-list/HLS start application response envelope，保留 legacy camera list body、stats、config success/list 與 HLS payload 欄位 | `b174ee2`, `1531478`, `b42d26a`, `7660fb8`, `9ef6f75`, current slice |
...
- Targeted camera HLS start vNext envelope test: `1 passed`
- Affected camera application/view tests: `111 passed`
