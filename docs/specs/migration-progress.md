...
| History path | history invalid-time-range、time-range-too-large、single-query timeout、batch timeout、single-query generic failure、single-query、batch 與 statistics application response envelope，保留 legacy `error` / `message` / `max_days` / history/statistics payload 欄位 | `4979988`, `be5a1e5`, `296da10`, `0e6db58`, `ae03e72`, `adc8619`, `66676c5`, `8eb4fc6` |
...
- History single-query failure envelope RED: targeted test failed with legacy-only `{"error": "history_query_failed", "message": "recorder unavailable"}`
- Targeted history single-query failure envelope test: `1 passed`
- Affected history/http tests: `97 passed`
- Full suite: `609 passed` on Python 3.14.6 / `mcr.microsoft.com/devcontainers/python:3.14-bookworm`
