...
| History path | history invalid-time-range、time-range-too-large、single-query timeout、batch timeout、single-query、batch 與 statistics application response envelope，保留 legacy `error` / `max_days` / history/statistics payload 欄位 | `4979988`, `be5a1e5`, `296da10`, `0e6db58`, `ae03e72`, `adc8619`, `66676c5` |
...
- History batch timeout envelope RED: targeted test failed with legacy-only `{"error": "query_timeout"}`
- Targeted history batch timeout envelope test: `1 passed`
- Affected history/http tests: `96 passed`
- Full suite: `608 passed` on Python 3.14.6 / `mcr.microsoft.com/devcontainers/python:3.14-bookworm`
