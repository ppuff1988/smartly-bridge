...
| 72 | `current slice` | Camera snapshot unavailable application response 補上 API vNext `schema_version`、`data.status`、`warnings`、`errors[]` envelope 欄位，同時保留 legacy `error` 欄位與 404 status | RED failed with missing `schema_version`; camera tests `119 passed`; full suite `552 passed` |
...
- Targeted camera snapshot unavailable vNext error envelope test: `1 passed`
