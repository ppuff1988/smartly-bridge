...
| 50 | current slice | Device event invalid action response 補上 API vNext `schema_version`、`data`、`warnings`、`errors[]` envelope 欄位，同時保留 legacy `error` / `message` 欄位 | RED failed with missing `schema_version`; affected tests `16 passed`; full suite `533 passed` |
...
| Event path | 新增 canonical event envelope、event deduplication，並為 accepted / duplicate / invalid action event response 補上 API vNext envelope fields | `3b54b65`, `42e0c61`, `e01355e`, `ddadb62`, current slice |
...
- Targeted invalid event vNext error envelope test: `1 passed`
- Affected event application/view tests: `16 passed`
