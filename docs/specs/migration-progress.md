...
| 66 | current slice | Camera HLS unsupported application response 補上 API vNext `schema_version`、`data.status`、`warnings`、`errors[]` envelope 欄位，同時保留 legacy `error` 欄位與 400 status | RED failed with missing `schema_version`; camera tests `115 passed`; full suite `548 passed` |
...
| Camera path | camera list/register/unregister/clear-cache/config-list/HLS start/info/stats/stop application response envelope 與 HLS unsupported error envelope，保留 legacy camera list body、stats、config success/list、HLS payload、stream info、stop 404 與 error 欄位 | `b174ee2`, `1531478`, `b42d26a`, `7660fb8`, `9ef6f75`, `77665f5`, `ede433d`, `ae647d9`, `9383ab8`, `8ec2d62`, current slice |
...
- Targeted camera HLS unsupported vNext error envelope test: `1 passed`
