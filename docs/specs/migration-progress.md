...
| 74 | `current slice` | WebRTC ICE entity-mismatch application response 補上 API vNext `schema_version`、`data.status`、`warnings`、`errors[]` envelope 欄位，同時保留 legacy `error` 欄位與 403 status | RED failed with missing `schema_version`; WebRTC tests `49 passed`; full suite `554 passed` |
...
| WebRTC path | WebRTC ICE session-not-found/entity-mismatch application response envelope，保留 legacy `error` 欄位 | `ff78eb5`, `current slice` |
...
- Targeted WebRTC ICE entity-mismatch vNext error envelope test: `1 passed`
- Affected WebRTC application/view tests: `49 passed`
