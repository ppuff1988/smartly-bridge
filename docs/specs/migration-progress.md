...
| 48 | current slice | Sync states response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy `states` / `logical_devices` 欄位 | RED failed with missing `schema_version`; affected tests `99 passed`; full suite `531 passed` |
...
| Sync aliases, warnings, and read path | logical devices 輸出 migration aliases、normalization warnings、API vNext response envelope，並支援 `use_logical_devices` read-path flag | `e47050c`, `040f769`, `4527bd5`, current slice |
