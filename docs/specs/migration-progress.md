...
| 60 | current slice | Camera config-list application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy `cameras` / `count` 欄位 | RED failed with missing `schema_version`; camera tests `110 passed`; full suite `543 passed` |
...
| Camera path | camera list/register/unregister/clear-cache/config-list application response envelope，保留 legacy camera list body、stats 與 config success/list 欄位 | `b174ee2`, `1531478`, `b42d26a`, `7660fb8`, current slice |
...
- Targeted camera config-list vNext envelope test: `1 passed`
- Affected camera application/view tests: `110 passed`
