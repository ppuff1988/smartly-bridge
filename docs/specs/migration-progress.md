...
| 84 | `current slice` | WebRTC offer answer application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy `type` / `sdp` / `session_id` 欄位 | RED failed with missing `schema_version`; WebRTC tests `51 passed`; full suite `558 passed` |
...
- Targeted WebRTC offer answer vNext envelope test: `1 passed`
