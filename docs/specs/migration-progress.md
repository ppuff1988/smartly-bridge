...
| 57 | current slice | Camera register application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy `success` / `action` / `entity_id` 欄位 | RED failed with missing `schema_version`; camera tests `107 passed`; full suite `540 passed` |
...
| Camera path | camera list/register application response envelope，保留 legacy camera list body、stats 與 config success 欄位 | `b174ee2`, current slice |
...
- Targeted camera register vNext envelope test: `1 passed`
- Affected camera application/view tests: `107 passed`
