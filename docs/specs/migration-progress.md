...
| 59 | current slice | Camera clear-cache application response 補上 API vNext `schema_version`、`data`、`warnings`、`errors` envelope 欄位，同時保留 legacy `success` / `action` / `cleared_count` 欄位 | RED failed with missing `schema_version`; camera tests `109 passed`; full suite `542 passed` |
...
| Camera path | camera list/register/unregister/clear-cache application response envelope，保留 legacy camera list body、stats 與 config success 欄位 | `b174ee2`, `1531478`, `b42d26a`, current slice |
